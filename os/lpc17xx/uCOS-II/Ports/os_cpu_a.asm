...
;  本文件用到的寄存器和宏                             
;*********************************************************************************************************                                
NVIC_INT_CTRL           EQU     0xE000ED04                              ;  Interrupt control state       PEND触发寄存器
                                                                        ;  register.
                                                                        ;  中断控制寄存器

NVIC_SYSPRI2            EQU     0xE000ED20                              ;  System priority register (2)  系统节拍与PEND优先级
                                                                        ;  系统优先级寄存器（2）

NVIC_PENDSV_PRI         EQU     0xFFFF0000                              ;  PendSV and system tick priority   设置系统节拍优先级最底
                                                                        ;  (Both are lowest,0xff)  
                                                                        ;  软件中断和系统节拍中断
                                                                        ;  （都为最低，0xff）.
NVIC_PENDSVSET          EQU     0x10000000                              ;  Value to trigger PendSV      PEND触发寄存器有效触发值
                                                                        ;  exception.触发软件中断的值.
...
OS_EXIT_CRITICAL
    ...
    IF OS_CRITICAL_INT_PRIO > 0                                         ;  开中断
        IT EQ
		MSREQ   BASEPRI, R1                                             ;  若被设成0 ，则不关闭任何中断
    ELSE
        IT EQ
		MSREQ   PRIMASK, R1                                             ;  CPSIE I  ;PRIMASK=0，  ;开中断
    ENDIF
        POP     {R1, R2}
        BX LR
...
OSStartHighRdy
        ...
        STR     R5, [R4]                                                ;  设置  系统节拍与PENDSV优先级为最底->32
